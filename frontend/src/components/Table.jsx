...
const Table = () => {
  ...
  const handleResult = (e) => {
    // 
    setResultado((e.target.value * valorTroca[2]).toFixed(2))
    ...
    <div id='Table-moedas'>
      ...
      <div id='lado_table'>
        <div id='tela-simulacao'>
          ...
          <p className='titulo-cota'>{valorTroca.length > 0 ? valorTroca[0].replace('/', ' para ') : 'Clique na tabela para poder simular.'}</p>

          <form onSubmit={handleNothing} className={valorTroca.length < 1 ? 'formDefault' : undefined}>

            <input id='input_number' type="number" 
              onChange={handleResult}
               />
            <img src="./static/convert.png" alt="convert"/>
              <p class="resultado">{resultado > 0 ? resultado : undefined}</p>
